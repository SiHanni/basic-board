...
export class PostsController {
  ...
  /**
   * 게시글 상세 조회 + 조회수 1 증가
   * (조회수 증가 로직이 없다면 나중에 PostsService에 getAndIncreaseViewCount 추가해서 연결)
   */
  @Get(':id')
  @ApiOperation({ summary: '게시글 상세 조회 (조회수 1 증가)' })
  @ApiOkResponse({ type: PostResponseDto })
  async getPost(@Param('id') id: string): Promise<PostResponseDto> {
    const post = await this.postsService.getAndIncreaseViewCount(id);
    return this.toPostResponseDto(post);
  }
